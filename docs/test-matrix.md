...
| ✓ | **Ubuntu 26.04 LTS** | GNOME 50 | Wayland | PyPI (pipx) | 2026-09-24 / v0.5.3 | Python 3.14, PySide6 6.11.2. Install, first-run wizard, tray, menu, in-app restart, Discord on `discord-ipc-0`, Apple Music in Google Chrome (deb), cover art, notifications. The tray needs no extra extension — Ubuntu ships one. Chrome needs Plasma Browser Integration, and the browser has to be restarted after installing the extension before the page shows up. Snap browsers publish no MPRIS at all (see FAQ). Chrome's own length grew past the real one mid-song (2:45 reported as 5:23) — fixed in 0.5.4, where Apple's length wins. |
...
| ✓ | **Debian 13** (Trixie) | GNOME 48 | Wayland | PyPI (pipx) | 2026-09-20 / v0.5.3 | Install, menu entry, tray with the AppIndicator extension, Discord, Firefox, cover, Recently played, developer mode. Without a tray Refrain stops without a word when started from the menu entry, although it has a dialog for a missing tray (under investigation). |
