...
| **Apple** (`itunes.apple.com` + artwork CDN, HTTPS) | If "Fetch album cover art" is on **and** a track with artist+title plays | The **artist and song title** (the title without "feat." or version tags; as a last try the title alone), a store country taken from your desktop language (e.g. `DE`, then `US`), a `User-Agent` containing the Refrain version, and your IP — to look up cover, length and Apple Music link; then the cover image is fetched. No album, no account, no auth, no cookies | **On** | Untick *Notifications → Fetch album cover art from iTunes* |
...
- The Apple cover-art lookup and the GitHub update check have their own
  ...
  network egress even in `Off` mode, also untick "Fetch album cover art"
